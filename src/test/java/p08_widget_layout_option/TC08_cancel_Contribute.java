...
public class TC08_cancel_Contribute extends ExtentReport{

	@Test
	public void cancelContribute() throws InterruptedException, MalformedURLException
	{
		tests = extent.createTest("cancel_Contribute");
        ...
        driver.findElement(By.xpath("//img[@title='Cancel']")).click();
        try {
        	String actual= driver.findElement(By.xpath("//span[contains(text(),'Popular Questions')]")).getText();
			String expected= "Popular Questions";
			Assert.assertEquals(actual, expected, "Test case fail -Assert fails expected key not matching");
			System.out.println("Test case pass- contribution cancelled sucessfully");

        }
        catch(Exception e) {
        	Assert.assertEquals(true, false, "Test case failed -Contribution not Cancelled");
        }
